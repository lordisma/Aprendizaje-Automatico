...
Para el aprendizaje de modelos predictores tanto para regresi�n como para problemas de clasificaci�n basados en diferentes paradigmas todos ellos tienen en com�n:

1. Obtienen un objeto de un tipo particular, o class, asociado. $class(objeto)$. Esta clase define su estructura: sus componentes y sus m�todos: las operaciones que se le pueden aplicar.
$methods(class(objeto))$.
2.Funciones gen�ricas, que se aplican sobre todo objeto

Para mostrar el modelo obtenido, ser� �til la funci�n
$print(objeto)$\\
Para la visualizaci�n del modelo $plot$\\
Para obtener un vector de predicciones $predict$\\
Para mostrar lo esencial del modelo aprendido $summary(objeto)$\\
M�s abajo se muestran unos ejemplos.


Adicionalmente podr�n usarse entre otras algunas de las funciones que se listan a continuaci�n junto con librer�as donde se las pueden encontrar. Indicar que no son �nicas

Las funciones lm y glm para modelos lineales,
que vienen en los paquetes standares de R como se ha visto anteriormente.
Se puede usar la librer�a caret para realiza Validacion cruzada
...
*Modelos de regresi�n con penalizacion*\\

en MASS est� disponible la funcion $lm.ridge()$ o bien, la funcion $enet()$ en el paquete elasticnet. Est�n disponibles tanto el lasso como el decay.
...
*Redes neuronales
la funci�n $nnet()$ del paquete *nnet* y el paquete RSNNS.

*arboles y modelos basados en �stos*
para Forest, bagging y boosting respectivamente:

funci�n $randomForest()$ del paquete randomForest\\
funcion $bagging()$ del paqete \\
funcion $gbm()$ \\

En muchas ocasiones es necesario pasar valores a algunos par�metros a los m�todos de aprendizaje, caso de no hacerlo �stos toman unos valores por defecto, ser� necesario conocer cu�l o cu�les. Ejemplo, para knn, necesario el valor $k$ de vecinos, para �rboles, profundidad, para bagging n�mero de �rboles etc...


Para ello ser� necesario, realizar el producto de los dominios de cada uno de los valores que se quieren evaluar. En el ejemplo los los par�metros interaction.depth, n.trees, y shrinkage  para la funci�n $gbm$
...
```
Se eligen los par�metros �ptimos de acuerdo a alguna de las medidas de error, del modelo.
...
Si se quiere afinar con la b�squeda del mejor par�metro a usar para un determinado modelo es com�n en muchos paquetes, una funci�n $tune$, o existe alguna gen�rica.
Seguimos con modelos b�sicos.
...
QuÃ© funciones se pueden aplicar sobre un modelo, como m4?
```{r}
methods(class=class(m4))
```

De las grÃ¡ficas anteriores parece que las relaciones observadas no son lineales ...

HabrÃ¡ que incorporar algÃºn tipo de transformaciÃ³n no lineal de los atributos ...
Por ejemplo, una forma cuadrÃ¡tica
...
Evaluacion exhaustiva, con $p$ pequeño
...
Para ver la grafica por tamaños y elegir un tamaño
```{r}
#par ( mfrow = c (1 ,2) )
...
Se ajusta un modelo lineal, por ejemplo mediante regresiÃ³n logÃ�???stica para predecir $mpg01$.
Se puede especificar de forma explÃ�???cita los atributos a considerar a la hora de construir el modelo, el resto se ignoran.
...
Una vez aprendido, veamos cÃ³mo predice...
...
predicciones con el modelo de regresiÃ³n logÃ�???stica
...
summary(ml2)
```
A la hora de comparar clasificadores, grÃ¡ficamente se muestra por la curva ROC
Es mejor clasificador, cuanto mayor sea el Ã¡rea debajo de la curva.
...
Se pueden hacer comparaciones grÃ¡ficas entre varios modelos
```{r}
...
Centrado, escalado, transformaciÃ³n para reducir la asimetria
Vamos a trabajar con el dataset segmentationOriginal   que trata de  
Cell Body Segmentation problema de clasificaciÃ³n , cÃ©lulas Pobremente segmentadas o Well segmentadas.
...
```
Se eliminan parte de la informaciÃ³n, columnas redundantes ... 
...
TransformaciÃ³n de atributos asimÃ©tricos, necesarios para la aplicaciÃ³n de algunos mÃ©todos de aprendizaje sensibles a distancias. Se consideran asimÃ©tricos cuando o bien la ratio entre min y max de $range() > 20$ o bien el valor skewness se aleja de 0.
...
Se quiere aplicar funciones sobre los datos para eliminar dicha asimetrÃ�???a para un trato homogÃ©neo de todas los atributos.

Una familia de funciones para la transformaciÃ³n ( que incluye desde cuadrÃ¡ticas, raÃ�???ces, inversas etc.. son las propuestas por Box y Cox (1964) un parÃ¡metro como  $\lambda$:
...
Con *skewness()* lo detecta pero cuÃ¡l es la transformaciÃ³n, para ello:
...
es justo,la transformaciÃ³n con $lambda = -0.9$
  Datos transformados:
...
tambiÃ©n en *caret*
Algunos redundantes o irrelevantes, es conveniente reducir dimensionalidad. El algoritmo PCA (principal components analysis) es un filtro (selector de caracterÃ�???sticas) no supervisado. Aunque es sensible a escala y valores grandes, previamente se hace el centrado y la escala.
...
```
Por filas vemos los atributos que forman parte de  cada uno de los componentes y sus coeficientes. Por defecto la funciÃ³n selecciona aquellos componentes que explican hasta el  95% de la variabilidad de los datos... se puede cambiar con argumentos thresh. 

A la hora de aplicar las transformaciones, en *caret* existe una funciÃ³n **preProcess()** que realiza todas transformaciones mencionadas de forma ordenada.
...
Eliminar las  variables con varianza 0 o muy prÃ³ximas, esto es muy desbalanceadas o de valor Ãºnico.
